    ...
    if (req.method === 'POST') {
        ...
        await helper.checkPermission(req, "v", async function (isPermited: any) {
            if (isPermited) {
                ...
                const { id, expenses_name, description, created_at } = req.body[0]
                console.log(`Looping..For..`, id)
                try {
                    const get_records: any = await query(`SELECT * FROM expenses_type WHERE expenses_name=?`, [expenses_name])
                    if (get_records.length == 0 || get_records.length == undefined) {
                        console.log(`Length...`, get_records.length, `ID Not Found Good For Insert For `, id, `On--------`, expenses_name)
                        ...
                        console.log(`Length...`, get_records.length, `Found ID Good For Update On`, get_records[0].id, `On--------`, expenses_name)
                        console.log(`...Now Show what you have...`, get_records)
                        const results: any = await query(
                            `UPDATE expenses_type SET 
                                        expenses_name = ?,
                                        description = ?,
                                        created_at = ? WHERE id = ? `,
                            [
