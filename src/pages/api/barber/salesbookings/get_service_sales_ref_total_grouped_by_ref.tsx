...
const handler: NextApiHandler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'GET') {
    try {
      //const {id} = req.query
      const get_records = await query(`SELECT ex_service_reference_id as id, ROUND(SUM(total_service_cost), 2) as itemValue FROM service_sales_ref GROUP BY ex_service_reference_id`)
