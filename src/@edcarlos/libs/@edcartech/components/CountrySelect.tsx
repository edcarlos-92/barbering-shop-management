...
export default function CountrySelect(props: any) {
  ...
  const { id, label, name, value, variant, width } = props

  const handleClick = (option) => {
    //setCountry(countries); 
    //console.log(`The Seleted Country is : `, countries.find(v => v.label[0]))
    console.log(`The Seleted Country is : `, option.Label)
  };
  ...
  return (
    ...
    <Autocomplete
      componentName={name}
      id={id}
      onChange={(event, value) => value?.label}
...
export function SelectCountry(props: any) {
  ...
  const { name, label, labelId, margin, type, id, error = null, onChange, size, sx, options, value, variant, ...other } = props;
  return (
    <>
      <Box sx={{ minWidth: 185 }} >
...
export function SelectCountryWVariant(props: any) {
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={onChange}>
            {
              options.map((item) => (
                <MenuItem key={item.code} value={item.label}>
                  <Box sx={{ display: 'flex', flexDirection: 'row', pl: 1, pt: 1 }} >
                    <span>
                      <img
                        loading="lazy"
                        width="20"
                    ...
                    <span style={{ marginLeft: 5, marginTop: -3 }}>{item.label}</span>
                  </Box>
