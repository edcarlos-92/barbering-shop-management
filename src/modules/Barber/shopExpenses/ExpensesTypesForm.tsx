...
export default function ExpensesTypesForm(props: any) {
  ...
  const validate = (fieldValues = values) => {
    let temp: any = { ...errors }
    if ('expenses_name' in fieldValues) temp.expenses_name = fieldValues.expenses_name.length != 0 ? "" : <RequiredFieldMessage />
  ...
  const [expenseName, setExpenseName] = useState(values.expenses_name);
  ...
  const handleRepeaterAddFields = () => {
    if (values.expenses_name !== '') {
      ...
      dispatch(silentInsert('barber/expenses/add_edit_expenses_types', repeaterInputField, ''));
  ...
  const sendToEditField = (item: any) => {
    ...
    repeaterInputField[0].expenses_name = item.expenses_name;
    repeaterInputField[0].description = item.description;
    setRepeaterInputFields([...repeaterInputField]);
    setExpenseName(item.expenses_name);
    setDescription(item.description);
  }

  const onDelete = (id: any) => {
    dispatch(silentDelete(`barber/expenses/delete_expenses_type?id=${id}`, ''));
    setConfirmDialog({
      ...confirmDialog,
      isOpen: false,
    })
  ...
  const getexpensesTypes = (() => {
    ...
    const { status, data, error, isFetching } = useQuery(
      'expensesTypes',
      async () => {
        const res = await axios.get(`/api/barber/expenses/get_expenses_types`)
        ...
        <Container key={repeaterItems.id} sx={{ mt: 2, mb: 5 }}>
          ...
          <FormControl sx={{ minWidth: 180, ml: { xs: 5, sm: 5 }, mb: { xs: 2, sm: 2 } }} >
            <Controls.TextInput
              id="expenses_name"
              name="expenses_name"
              label={<IntlMessages id='expenses_name.label' />}
              value={expenseName}
            ...
            expensesInfo !== null || expensesInfo !== [] ? (
              <TableContainer component={Paper} sx={{ mb: 3 }}>
                <Table sx={{ minWidth: 450 }} size="small" aria-label="a dense table">
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ fontWeight: Fonts.BOLD }} align="left" ><IntlMessages id="expense_id.short" /></TableCell>
                      <TableCell sx={{ fontWeight: Fonts.BOLD }} align="left"><IntlMessages id="expense_name.short" /></TableCell>
                  ...
                  <TableBody>
                    {expensesInfo.map((item: any) => (
                      ...
                      <TableRow key={item.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        ...
                        <TableCell align="left" >{item.expenses_name}</TableCell>
